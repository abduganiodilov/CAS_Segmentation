...
def cmd_train(
    ...
    val_tfms = A.Compose([A.Resize(img_size, img_size), ToTensorV2()])
    ...
    class SliceDS(Dataset):
        ...
        def __getitem__(self, i):
            ...
            img = cv2.imread(ip, cv2.IMREAD_GRAYSCALE).astype("float32") / 255.0
            ...
            if img.ndim == 3 and img.shape[0] not in (1, 3): img = img.permute(2, 0, 1).contiguous()
            if msk.ndim == 3 and msk.shape[0] != 1:        msk = msk.permute(2, 0, 1).contiguous()
            return img, msk

    pin = (device.type == "cuda")
    train_dl = DataLoader(SliceDS(train_df, train_tfms), batch_size=batch_size, shuffle=True,
                          num_workers=workers, pin_memory=pin)
    val_dl = DataLoader(SliceDS(val_df, val_tfms), batch_size=batch_size, shuffle=False,
                        num_workers=workers, pin_memory=pin)
    ...
    def iou_score(logits, targets, th=0.5, eps=1e-7):
        ...
        t = targets.float()
        inter = (p * t).sum((1, 2, 3));
        union = p.sum((1, 2, 3)) + t.sum((1, 2, 3)) - inter
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        model.eval();
        va_loss = va_iou = va_dice = n_va = 0.0
        with torch.no_grad():
        ...
        improved = va_iou > best_iou + min_delta
        if improved:
            best_iou = va_iou
            no_improve = 0
...
def cmd_eval(slices_dir: Path, run_dir: Path, img_size: int, batch_size: int, workers: int, save_grid: bool):
    ...
    class SliceDS(Dataset):
        ...
        def __getitem__(self, i):
            ...
            img = cv2.imread(ip, cv2.IMREAD_GRAYSCALE).astype("float32") / 255.0
            msk = (cv2.imread(mp, cv2.IMREAD_GRAYSCALE) > 127).astype("float32")
            out = self.tfm(image=img[..., None], mask=msk[..., None]);
            img, msk = out["image"], out["mask"]
            if img.ndim == 3 and img.shape[0] not in (1, 3): img = img.permute(2, 0, 1).contiguous()
            if msk.ndim == 3 and msk.shape[0] != 1:       msk = msk.permute(2, 0, 1).contiguous()
            return img, msk
    ...
    if save_grid:
        ...
        with torch.no_grad():
            for i, idx in enumerate(idxs, 1):
                img, msk = val_ds[idx]
                prob = torch.sigmoid(model(img.unsqueeze(0).to(device))).cpu().squeeze().numpy()
                pred = (prob > 0.5).astype(np.uint8)
                plt.subplot(k, 3, 3 * (i - 1) + 1);
                plt.imshow(img.squeeze().numpy(), cmap='gray');
                plt.title('Image');
                plt.axis('off')
                plt.subplot(k, 3, 3 * (i - 1) + 2);
                plt.imshow(msk.squeeze().numpy(), cmap='gray');
                plt.title('Mask');
                plt.axis('off')
                plt.subplot(k, 3, 3 * (i - 1) + 3);
